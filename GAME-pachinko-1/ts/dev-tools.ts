...
(function setupDevTools() {
	...
	function injectTimeScaleSlider() {
		...
		paddleLabel.appendChild(
			document.createTextNode("パドルを動かす（2つ一括）"),
		);
	...
	function injectSensorCounters() {
		...
		if (counterIds.length === 0) {
			...
			counterIds.forEach((id) => {
				...
				optNone.value = "";
				optNone.text = "none";
				const optEnter = document.createElement("option");
				optEnter.value = "enter";
				optEnter.text = "enter";
				const optExit = document.createElement("option");
				optExit.value = "exit";
				optExit.text = "exit";
				sel.appendChild(optNone);
				sel.appendChild(optEnter);
				sel.appendChild(optExit);
				const cur =
					data && data.removeOn
						? String(data.removeOn)
						: data && data.removeOnPass
							? "exit"
							: "";
				sel.value = cur;
							...
							window.dispatchEvent(
								new CustomEvent("devtools:sensor-updated", {
									detail: {
										id,
										type: "config",
										counter: Object.assign({}, counters[id]),
									},
								}),
							);
						}
				...
				const pmOptBall = document.createElement("option");
				pmOptBall.value = "ball";
				pmOptBall.text = "particle: ball";
				const pmOptCustom = document.createElement("option");
				pmOptCustom.value = "custom";
				pmOptCustom.text = "particle: custom";
				const pmOptDefault = document.createElement("option");
				pmOptDefault.value = "default";
				pmOptDefault.text = "particle: default";
				pmSel.appendChild(pmOptBall);
				pmSel.appendChild(pmOptCustom);
				pmSel.appendChild(pmOptDefault);
				const curPm =
					data && data.particleMode
						? String(data.particleMode)
						: data && data.particleColor
							? "custom"
							: "ball";
				pmSel.value = curPm;
	...
	function injectTotalSpawned() {
		...
		function refresh() {
			...
			if (el && cfgNow && cfgNow.metrics)
				el.textContent = String(cfgNow.metrics.totalSpawned ?? 0);
		}
		// 最初の表示反映
		refresh();
	...
	function updateSensorCounters() {
		const cfg = getCfg();
		// センサー情報の更新
		if (cfg && cfg.sensorCounters && cfg.sensorCounters.enabled)
	...
	if (window.__engine_for_devtools__)
		CURRENT_ENGINE = window.__engine_for_devtools__;
	if (ensureDevPanel()) {
		injectTimeScaleSlider();
		injectTotalSpawned();
		injectSensorCounters();
