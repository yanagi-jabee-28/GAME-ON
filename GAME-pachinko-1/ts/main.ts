...
import {
	...
	createPolygon,
	createRectangle,
	createRotatingYakumono,
	createSensorCounter,
	createSensorCounterPolygon,
	getOffsets,
	loadPegs,
} from "../ts/objects";
...
function pachiInit() {
	...
	function pickParticleColor(cfg, ballBody) {
		try {
			...
			if (mode === "ball")
				return ballBody && ballBody.render && ballBody.render.fillStyle
	...
	engine.positionIterations = Number(
		GAME_CONFIG.physics?.positionIterations ?? 12,
	);
	engine.velocityIterations = Number(
		GAME_CONFIG.physics?.velocityIterations ?? 8,
	);
	engine.constraintIterations = Number(
		GAME_CONFIG.physics?.constraintIterations ?? 6,
	);
	...
	engine.world.gravity.y = Number(
		GAME_CONFIG.physics?.gravityY ?? engine.world.gravity.y,
	);
	...
	function showLoadingOverlay() {
		try {
			...
			try {
				...
				style.textContent =
					"@keyframes pachi-spin{from{transform:rotate(0)}to{transform:rotate(360deg)}}";
	...
	try {
		...
		if (!links.length) {
			...
			const markLoaded = () => {
				remaining--;
				if (remaining <= 0) stylesLoaded = true;
			};
	...
	function ensureCanvasSized() {
		try {
			...
			const ch = Math.max(
				0,
				Math.round(container.clientHeight || rect.height || 0),
			);
			if (cw <= 0 || ch <= 0) return;
			...
			if (c.width !== pw || c.height !== ph) {
				...
				try {
					if (render && render.options) render.options.pixelRatio = dpr;
				} catch (_) {}
				...
				try {
					if (render && render.canvas) {
						render.canvas.width = pw;
						render.canvas.height = ph;
						render.canvas.style.width = String(cw) + "px";
						render.canvas.style.height = String(ch) + "px";
					}
				} catch (_) {}
				console.debug("[PACHINKO] ensureCanvasSized ->", {
					cw,
					ch,
					dpr,
					logicalW,
					...
					ph,
					renderOptions:
						render && render.options ? Object.assign({}, render.options) : null,
				});
			}
			...
			const now =
				typeof performance !== "undefined" && performance.now
					...
					: Date.now();
			const sizeChanged =
				Math.abs(cw - lastCw) > 0.5 || Math.abs(ch - lastCh) > 0.5;
			if (sizeChanged) {
			...
			const stableOk =
				layoutStableSince &&
				now - layoutStableSince >= stableWindowMs &&
			...
			console.warn("[PACHINKO] ensureCanvasSized error", e);
		}
	}
		...
		const getLayer = (b) => {
			const v =
				b && b.render && typeof b.render.layer === "number"
					? b.render.layer
					: b && b.render && b.render.layer != null
						? Number(b.render.layer)
						: 1;
	...
	(function setupAdaptivePhysics() {
		...
		if (lowMemDevice) {
			...
			try {
				GAME_CONFIG.physics.substeps = Math.max(
					1,
					Number(GAME_CONFIG.physics.substeps || 1),
				);
				GAME_CONFIG.physics.fixedFps = Math.max(
					30,
					Number(GAME_CONFIG.physics.fixedFps || 60),
				);
				GAME_CONFIG.physics.positionIterations = Math.max(
					6,
					Number(GAME_CONFIG.physics.positionIterations || 6),
				);
				GAME_CONFIG.physics.velocityIterations = Math.max(
					4,
					Number(GAME_CONFIG.physics.velocityIterations || 4),
				);
				GAME_CONFIG.physics.constraintIterations = Math.max(
					2,
					Number(GAME_CONFIG.physics.constraintIterations || 2),
				);
		...
		(window as any).__recordPhysicsPerf__ = (frameMs) => {
			try {
				...
				if (avg > 30) {
					// avg > 30ms: 平均フレーム時間が30msを超える（約33fps未満） -> 負荷高めと判断
					// degrade gradually (avoid sudden jumps)
					GAME_CONFIG.physics.substeps = Math.max(
						1,
						(Number(GAME_CONFIG.physics.substeps) || 1) - 1,
					);
					GAME_CONFIG.physics.fixedFps = Math.max(
						30,
						(Number(GAME_CONFIG.physics.fixedFps) || 60) - 5,
					);
					GAME_CONFIG.physics.positionIterations = Math.max(
						4,
						(Number(GAME_CONFIG.physics.positionIterations) || 6) - 1,
					);
					GAME_CONFIG.physics.velocityIterations = Math.max(
						3,
						(Number(GAME_CONFIG.physics.velocityIterations) || 4) - 1,
					);
					// force simple renderer
					if (render && render.options) render.options.pixelRatio = 1;
				} else if (avg < 12) {
					// avg < 12ms: 平均フレーム時間が12ms未満（約83fps以上） -> 余裕があるため品質回復可
					// restore slowly toward defaults
					GAME_CONFIG.physics.substeps = Math.min(
						4,
						(Number(GAME_CONFIG.physics.substeps) || 1) + 1,
					);
					GAME_CONFIG.physics.fixedFps = Math.min(
						60,
						(Number(GAME_CONFIG.physics.fixedFps) || 30) + 5,
					);
					GAME_CONFIG.physics.positionIterations = Math.min(
						12,
						(Number(GAME_CONFIG.physics.positionIterations) || 6) + 1,
					);
					GAME_CONFIG.physics.velocityIterations = Math.min(
						8,
						(Number(GAME_CONFIG.physics.velocityIterations) || 4) + 1,
					);
				}
	...
	(function runFixedTimestep() {
		...
		function loop(now) {
			...
			while (acc >= fixedDtMs && steps < maxFixedStepsPerFrame) {
				...
				const effSubsteps =
					adaptiveSubsteps && elapsed > fixedDtMs * 1.5 ? 1 : substeps;
					...
					if (rot.mode === "program" && rot.program) {
						...
						if (typeof rot.inertial.springStiffness !== "number") {
							...
							try {
								...
								if (distSq > 2.25) {
									...
									Matter.Body.translate(rot.body, {
										x: dx * frac,
										y: dy * frac,
									});
								}
						...
						if (I > 0) {
							...
							if (gBias && Math.abs(gBias) > 0) {
								...
								const gravityOffset = Number(
									rot.inertial.gravityOffset ?? rot.inertial.gravityRadius ?? 8,
								);
								const worldG =
									engine &&
									engine.world &&
									typeof engine.world.gravity?.y === "number"
										? Math.abs(engine.world.gravity.y)
										: 1;
								gravityTorque =
									-gBias *
									mass *
									gravityOffset *
									worldG *
									Math.sin(cur - gravityTarget);
								if (!Number.isFinite(gravityTorque)) gravityTorque = 0;
								gravityTorque = Math.max(
									-10000,
							...
							try {
								const approxR = Math.max(
									8,
									(rot.body.bounds.max.x - rot.body.bounds.min.x) / 4,
								);
	...
	if (GAME_CONFIG.topPlate?.enabled) {
		GAME_CONFIG.topPlate.radius =
			GAME_CONFIG.topPlate.radius || Math.round(width * 0.6);
	}

	// create and add bounds (createBounds now returns an array)
	const currentBounds = createBounds();
	addBoundsToWorld(currentBounds, world);
	// pegs preset path from config
	try {
		const pegsPath =
			(GAME_CONFIG.presets && GAME_CONFIG.presets.pegs) ||
			"pegs-presets/pegs3.json";
	...
	const { xOffset: globalXOffset, yOffset: globalYOffset } =
	...
	if (typeof window !== "undefined") {
		window.setRotatorEnabledById = (id, enabled) => {
			const r = Array.isArray(rotators)
				? rotators.find((r) => r && r.id === id)
				: null;
			if (r) r.enabled = !!enabled;
			return !!(r && r.enabled);
		};
		window.setRotatorEnabledByIndex = (index, enabled) => {
		...
		window.setAllRotatorsEnabled = (enabled) => {
		...
		window.toggleRotatorEnabled = (idOrIndex) => {
			if (typeof idOrIndex === "number") {
				const i = idOrIndex | 0;
				if (!rotators[i]) return false;
				rotators[i].enabled = !rotators[i].enabled;
				return rotators[i].enabled;
			}
			const r = Array.isArray(rotators)
				? rotators.find((r) => r && r.id === String(idOrIndex))
				: null;
			if (!r) return false;
			r.enabled = !r.enabled;
			return r.enabled;
		};
		window.setRotatorsEnabledByKind = (kind, enabled) => {
			if (!Array.isArray(rotators)) return 0;
			const k = String(kind || "").toLowerCase();
			let n = 0;
			for (const r of rotators) {
				if (!r) continue;
				if (String(r.kind).toLowerCase() === k) {
					r.enabled = !!enabled;
					n++;
				}
			}
	...
	function initRotatorsFromPreset(preset) {
		...
		return items
			.map((item, idx) => {
				...
				const bladeColor =
					item.bladeColor ??
					item.render?.fillStyle ??
					windmillConfig.bladeColor ??
					windmillConfig.render?.fillStyle;
				const centerColor =
					item.centerColor ??
					item.centerFill ??
					windmillConfig.centerColor ??
					windmillConfig.centerFill;
				const render = Object.assign({}, item.render || {});
				if (item.layer != null && render.layer == null)
					render.layer = Number(item.layer);
				const blueprint = {
				...
				const id = String(
					item.id || `${isPaddle ? "paddle" : "windmill"}_${idx}`,
				);
				...
				if (
					rotCfg &&
					Array.isArray(rotCfg.sequence || rotCfg.keyframes || rotCfg.waypoints)
				) {
						...
						.map((s) => {
							if (typeof s === "number") {
								return {
									angleDeg: s,
									moveMs: defMove,
									holdMs: Number.isFinite(defHold) ? defHold : 300,
								};
							}
							const angleDeg = Number(
								s.angleDeg ?? s.deg ?? s.angle ?? s.a ?? 0,
							);
							const moveMs = Number(s.moveMs ?? defMove);
							const holdMs = Number.isFinite(s.holdMs)
								? Number(s.holdMs)
								: Number.isFinite(defHold)
									? defHold
									: 300;
					...
					if (offsetMs) {
						let remain = loop
							? offsetMs % totalCycleMs
							: Math.min(offsetMs, totalCycleMs);
						while (remain > 0 && !(program.completed && !loop)) {
							...
							if (program.phase === "hold") {
								...
								remain -= use;
								if (program.phaseElapsed >= st.holdMs) {
									program.phase = "move";
									program.phaseElapsed = 0;
								}
							} else {
							...
							if (
								!loop &&
								program.curIndex === steps.length - 1 &&
								program.phase === "hold" &&
								program.phaseElapsed >= steps[program.curIndex].holdMs
							) {
				...
				if (rotCfg && Number.isFinite(rotCfg.durationMs || rotCfg.duration)) {
					const startDeg =
						Number(
							rotCfg.startDeg ??
								rotCfg.fromDeg ??
								rotCfg.startAngleDeg ??
								rotCfg.from,
						) || 0;
					const endDeg = Number(
						rotCfg.endDeg ?? rotCfg.toDeg ?? rotCfg.endAngleDeg ?? rotCfg.to,
					);
					const hasEnd = Number.isFinite(endDeg);
					const durationMs =
						Number(rotCfg.durationMs ?? rotCfg.duration) || 1000;
				...
				const anglePerSecond =
					rps * 2 * Math.PI * (item.direction === -1 ? -1 : 1);
				return {
					id,
					kind,
					body,
	...
	function initPolygonsFromPreset(preset) {
		...
		const defMode =
			...
			preset.defaults?.polygons?.coordMode ||
		...
		const bodies = items
			.map((p) => {
				const coordMode =
					p.coordMode ||
					p.pointsMode ||
					(p.useWorldPoints ? "world" : undefined) ||
					defMode ||
					"local";
	...
	function initDecorPolygonsFromPreset(preset) {
		const items = Array.isArray(preset.decorPolygons)
			...
			: [];
		const defMode =
			preset.decorPolygonsDefaults?.coordMode ||
			preset.defaults?.decorPolygons?.coordMode ||
		...
		const bodies = items
			.map((s) => {
				const coordMode =
					s.coordMode ||
					s.pointsMode ||
					(s.useWorldPoints ? "world" : undefined) ||
					"local";
				if (String(coordMode).toLowerCase() === "world") {
					// world座標指定: points にオフセットを適用
					const pts = Array.isArray(s.points)
						...
						: s.points;
					return createSensorCounterPolygon(
						Object.assign({}, s, { points: pts }),
					);
				}
				// local座標指定: x,y にオフセット
				return createSensorCounterPolygon(
	...
	(async () => {
		try {
			const objPath =
				(GAME_CONFIG.presets && GAME_CONFIG.presets.objects) ||
				"objects-presets/default.json";
			const res = await fetch(objPath);
			if (!res.ok)
				throw new Error(
					`Failed to load objects preset: ${res.status} ${res.statusText}`,
				);
	...
	let holdAccumMs = 0;
	let holdIntervalMsCfg = Number(
		(GAME_CONFIG.launch && GAME_CONFIG.launch.holdIntervalMs) || 300,
	);
	let holdFirstDelayMsCfg = Number(
		(GAME_CONFIG.launch && GAME_CONFIG.launch.holdFirstShotDelayMs) || 0,
	);
	...
	function computeSpawnCoords() {
		...
		const { xOffset: sxOff, yOffset: syOff } =
			typeof getOffsets === "function"
				...
				: { xOffset: 0, yOffset: 0 };
		const startX =
			typeof spawnCfg.x === "number" ? spawnCfg.x + sxOff : 40 + sxOff;
		const startY =
	...
	function updateLaunchPadPosition() {
		...
		const angleEl = /** @type {HTMLInputElement|null} */ (
			document.getElementById("angle-slider")
		);
		const angleDeg = Number(
			angleEl
				? (angleEl as HTMLInputElement).value
				: GAME_CONFIG.launch?.defaultAngle || 90,
		);
		const offsetY = padCfgAny.offsetY || 0;
		// 近端中心を原点にする: launch point からのオフセットを回転座標に沿って適用
		Matter.Body.setPosition(launchPadBody, {
			x: p.x - originX,
			y: p.y - originY,
		});
						...
						renderedCanvas:
							render && render.canvas
								? {
										...
										styleW: render.canvas.style.width,
										styleH: render.canvas.style.height,
									}
								: null,
					});
				}
	...
	try {
		window.addEventListener("load", () => tryInitPosition(2, 120));
	} catch (_) {}
	...
	try {
		setTimeout(() => {
			console.debug("[PACHINKO] delayed fallback sizing");
			tryInitPosition(1, 120);
			try {
				ensureCanvasSized();
			} catch (_) {}
		}, 600);
	...
	const angleSlider = /** @type {HTMLInputElement | null} */ (
		document.getElementById("angle-slider")
	);
	/** @type {HTMLInputElement | null} */
	const speedSlider: HTMLInputElement | null = document.getElementById(
		"speed-slider",
	...
	function updateArrow() {
		...
		const angle = Number(
			(angleSlider as HTMLInputElement)?.value ??
	...
	const cfgAmmo =
		GAME_CONFIG &&
		GAME_CONFIG.launch &&
		Number.isFinite(Number(GAME_CONFIG.launch.ammo))
			? Number(GAME_CONFIG.launch.ammo)
			: 1000;
	...
	function updateAmmoUI() {
		try {
			...
			if (ammoEl) {
				...
				// 低弾閾値: 10 発以下で警告スタイル
				if (launchAmmo <= 10) ammoEl.classList.add("low");
		...
		window.addEventListener("slot:win", (ev) => {
			try {
				...
				if (gain > 0) {
					...
					try {
						window.dispatchEvent(
							new CustomEvent("devtools:ammo-gained", {
								detail: {
									source: "slot:win",
									amount,
									mult,
				...
				try {
					...
					if (templ) {
						...
						showToastMessage(
							msg,
							Number(GAME_CONFIG?.rewards?.slotWinMessageMs) || 2200,
						);
					}
				} catch (_) {
					/* no-op */
	...
	function spawnBallFromUI() {
		...
		let angleDeg = Number(
			(angleSlider && (angleSlider as HTMLInputElement).value) ||
				GAME_CONFIG.launch?.defaultAngle ||
				90,
		);
		...
		const velocity = {
			...
			y: -Math.sin(angleRad) * speedPxPerSec,
		};
		...
		// 総射出数をインクリメント
		try {
			if (GAME_CONFIG && GAME_CONFIG.metrics)
				GAME_CONFIG.metrics.totalSpawned =
					(Number(GAME_CONFIG.metrics.totalSpawned) || 0) + 1;
			// 開発者ツール向けに発射イベントを通知（即時更新を促す）
			try {
				if (typeof window !== "undefined" && window.dispatchEvent) {
					window.dispatchEvent(
						new CustomEvent("devtools:ball-spawned", {
							detail: { total: GAME_CONFIG.metrics.totalSpawned },
						}),
					);
				}
			} catch (_) {
	...
	Events.on(engine, "collisionStart", (event) => {
		...
		for (const pair of pairs) {
			...
			try {
				...
				let disable = false;
				if (bodyA.label === ballLabel)
					disable = shouldDisableByOneWay(bodyA, bodyB);
				else if (bodyB.label === ballLabel)
					disable = shouldDisableByOneWay(bodyB, bodyA);
				if (disable) {
			...
			try {
				if (Array.isArray(rotators) && rotators.length) {
					...
					const map = new Map(
						rotators.filter(Boolean).map((r) => [r.body.id, r]),
					);
					let ballBody = null,
						rot = null;
					if (bodyA.label === ballLabel && map.has(bodyB.id)) {
						ballBody = bodyA;
						rot = map.get(bodyB.id);
					} else if (bodyB.label === ballLabel && map.has(bodyA.id)) {
						ballBody = bodyB;
						rot = map.get(bodyA.id);
					}
			...
			function handleFloorHit(ballBody) {
				...
				if (!(eff && eff.removeBall === false)) {
					...
					if (typeof window !== "undefined" && window.dispatchEvent) {
						window.dispatchEvent(
							new CustomEvent("devtools:ball-removed", {
								detail: { ballId: ballBody.id, trigger: "floor" },
							}),
						);
					}
				}
	...
	function handleSensorCounterCollision(bodyA, bodyB, eventType) {
		...
		// センサーとボールのペアを特定
		if (
			bodyA.label === ballLabel &&
			bodyB.label &&
			bodyB.label.startsWith("sensor_counter_")
		) {
			ballBody = bodyA;
			sensorBody = bodyB;
		} else if (
			bodyB.label === ballLabel &&
			bodyA.label &&
			bodyA.label.startsWith("sensor_counter_")
		) {
			ballBody = bodyB;
			sensorBody = bodyA;
		}
		...
		if (eventType === "enter") {
			...
			if (!sensorBody.sensorData.isEntered.has(ballId)) {
				...
				try {
					if (typeof window !== "undefined" && window.dispatchEvent) {
						window.dispatchEvent(
							new CustomEvent("devtools:sensor-updated", {
								detail: {
									id: counterId,
									type: "enter",
				...
				try {
					const gain =
						GAME_CONFIG &&
						GAME_CONFIG.launch &&
						Number.isFinite(Number(GAME_CONFIG.launch.ammoGainOnSensor))
							? Number(GAME_CONFIG.launch.ammoGainOnSensor)
							: 0;
					if (gain > 0 && typeof launchAmmo === "number") {
						...
						try {
							const templ =
								(GAME_CONFIG &&
									GAME_CONFIG.rewards &&
									GAME_CONFIG.rewards.sensorEnterMessageTemplate) ||
								"";
							if (templ) {
								const msg = String(templ).replaceAll("{gain}", String(gain));
								showToastMessage(
									msg,
									Number(GAME_CONFIG?.rewards?.slotWinMessageMs) || 1200,
								);
							}
						} catch (_) {
							/* no-op */
				...
				try {
					...
					const removeOn =
						cfgEntry.removeOn || (cfgEntry.removeOnPass ? "exit" : null);
					if (removeOn === "enter") {
						if (
							world &&
							ballBody &&
							world.bodies &&
							world.bodies.indexOf &&
							world.bodies.indexOf(ballBody) !== -1
						) {
							try {
			...
			if (sensorBody.sensorData.isEntered.has(ballId)) {
				...
				try {
					if (typeof window !== "undefined" && window.dispatchEvent) {
						window.dispatchEvent(
							new CustomEvent("pachi:hit", {
								detail: { counterId, totalPassed: counter.totalPassed },
							}),
						);
					}
				} catch (_) {
