...
//    cat mypreset.json | node scripts/write-preset.js default.json
// 2) Or provide an input file then output name:
//    node scripts/write-preset.js default.json input.json

const fs = require('fs');
const path = require('path');

function usage() {
	console.error('Usage: node scripts/write-preset.js <out-filename.json> [in-file.json]');
	process.exit(1);
}
...
const outDir = path.join(__dirname, '..', 'src', 'pegs-presets');
