...
class SlotGame {
	...
	updateBetConstraints() {
		...
		const _el = el as HTMLInputElement;
		let cur = Number(String(_el.value ?? '').replace(/[\,\s]+/g, '')) || 0;
		if (cur > effectiveMax) {
	...
	_cacheFinanceElements() {
		...
		if (this.elStepUp) {
			...
			this.elStepUp.addEventListener('pointerdown', (e) => {
				...
				try { el?.setPointerCapture?.((e as any).pointerId); } catch (err) { }
			...
			this.elStepUp.addEventListener('pointerup', (e) => {
				...
				const el = e.currentTarget as any;
				try { el?.releasePointerCapture?.((e as any).pointerId); } catch (err) { }
