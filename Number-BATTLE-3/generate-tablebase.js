...
function applyMove(state, move) {
	...
	if (move.type === "attack") {
		const handsToUpdate =
			move.to === "player" ? nextState.playerHands : nextState.aiHands;
		const attackerValue = (
			move.from === "player" ? state.playerHands : state.aiHands
		)[move.fromIndex];
		handsToUpdate[move.toIndex] =
			(handsToUpdate[move.toIndex] + attackerValue) % 5;
	} else {
		// split
			...
			if (P.outcome === OUTCOME.UNKNOWN) {
				...
				if (P.unknown_successors_count === 0) {
					...
					const maxDist = Math.max(
						...P.successors.map((succ) => succ.distance),
					);
...
// 6. 結果をJSONファイルに書き出し
fs.writeFileSync(
	"chopsticks-tablebase.json",
	JSON.stringify(finalTable, null, 2),
);

console.log(
	"Final retrograde analysis complete. Tablebase saved to chopsticks-tablebase.json",
);
