...
const playerHandLeftEl = document.getElementById(
	"player-hand-left",
) as HTMLElement;
const playerHandRightEl = document.getElementById(
	"player-hand-right",
) as HTMLElement;
...
const thumbsControls = document.getElementById(
	"thumbs-controls",
) as HTMLElement;
...
let playerScore = 0;
let cpuScore = 0;
let playerChoice: { thumbs: number | null; call: number | null } = {
	thumbs: null,
	call: null,
};
...
function checkCanPlay() {
	...
	if (currentTurn === "player") {
		playButton.disabled =
			playerChoice.thumbs === null || playerChoice.call === null;
	} else {
		// CPU's turn
...
playButton.addEventListener("click", () => {
	...
	if (currentTurn === "player") {
		...
		possibleCalls = possibleCalls.filter(
			(n) => n >= cpuThumbs && n <= cpuThumbs + playerHands,
		);
