		...
		try {
			if (Array.isArray(src)) {
				...
				audio.addEventListener("error", () => {
					console.warn(
						"Sound file(s) failed to load, will use synthetic if available:",
						src,
					);
				...
				audio.addEventListener("error", () => {
					console.warn(
						"Sound file failed to load, will use synthetic if available:",
						src,
					);
					delete this.sounds[key];
				});
	...
	_prepareSynthetic() {
		...
		this.synthetic["click_var_1"] = () => {
			...
			const buffer = ctx.createBuffer(
				1,
				Math.floor(ctx.sampleRate * length),
				ctx.sampleRate,
			);
